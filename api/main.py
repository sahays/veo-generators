...
# Paths non-master accounts can write to (auth + file uploads)
NON_MASTER_WRITE_ALLOWED = {"/api/v1/auth/validate"}
NON_MASTER_WRITE_PREFIXES = ("/api/v1/assets/upload",)
...
class InviteCodeMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Non-master accounts are read-only: block POST/PATCH/DELETE except allowed paths
        if not request.state.is_master and request.method in (
            "POST",
            "PATCH",
            "DELETE",
        ):
            allowed = path in NON_MASTER_WRITE_ALLOWED or path.startswith(
                NON_MASTER_WRITE_PREFIXES
            )
            if not allowed:
                logger.warning(
                    f"Auth: non-master write blocked for {request.method} {path}"
                )
                return JSONResponse(
                    status_code=403,
